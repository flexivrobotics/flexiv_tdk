...
![](../assets/settings_remote_mode.png)
To enable Remote Mode, turn on the toggle, then select Ethernet from the Select a Mode
drop-down list
![remote_mote_ethernet](../assets/remote_mote_ethernet.png)

View the currently applied Ethernet communication protocol(s). If no license has been installed, you can install the RDK/TDK license in Settings >
License.
...
For LAN teleoperation, please set both robots and the user's computer to the same network segment with different IP Addresses (e.g., 192.168.2.11x with a subnet mask of 255.255.255.0).
