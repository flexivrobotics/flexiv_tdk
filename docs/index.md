...
This source tree is **TDK v1.6.4**. The C++ library is a self-contained shared object whose only public dependency is **flexiv_rdk 1.9.4**. The Python package still requires **flexivrdk==1.9.4**. The hosted GitHub Pages site is TDK v2.x; preview these pages locally with `mkdocs serve`.
